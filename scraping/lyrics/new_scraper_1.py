...
async def write_result(id_, artist, writer,writer2):
    async with asyncio.Lock():
        if isinstance(artist,str): 
            pylyrics = asynclyrics.AsyncLyrics()  
            lyrics = await pylyrics.get_artist_lyrics(artist)
...
async def bound_fetch(id_,artist, writer, sem,writer2):
    # Getter function with semaphore.
    #since fxn has to pass in here we are blocking by 4 max semifore
    
    async with sem:
        await write_result(id_, artist, writer,writer2)
...
async def main(ids,names,i):
    ...
    with open(f'lyrics_scrape_{i}.csv', 'a') as success, open('fails.csv', 'a') as fail:
        ...
        tasks = [bound_fetch(id_,artist,writer,sem,failwriter) for id_,artist in zip(ids,names)]
